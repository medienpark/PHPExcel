...
*/
     ...
     */
    private function convert(mixed $token)
    {
     ...
     */
    private function convertNumber(mixed $num)
    {
     ...
     */
    private function match(mixed $token)
    {
        switch ($token) {
        case "+":
        case "-":
    ...
    private function fact()
    {
        ...
        if (preg_match('/^\$?[A-Ia-i]?[A-Za-z]\$?[0-9]+$/', $this->currentToken)) {
            ...
            $this->advance();
            return $result;
        } elseif (preg_match('/^(\$)?[A-Ia-i]?[A-Za-z](\$)?[0-9]+:(\$)?[A-Ia-i]?[A-Za-z](\$)?[0-9]+$/', $this->currentToken) 
     ...
     * @return array A tree
     */
    private function createTree(mixed $value, mixed $left, mixed $right)
    {
