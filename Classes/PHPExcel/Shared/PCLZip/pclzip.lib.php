...
class PclZip
{
    ...
    public function create($p_filelist)
    {
        ...
        if ($v_size > 1) {
            ...
            if ((is_integer($v_arg_list[0])) && ($v_arg_list[0] > 77000)) {
                // ----- Parse the options
                $v_result = $this->privParseOptions($v_arg_list, $v_size, $v_options, [PCLZIP_OPT_REMOVE_PATH => 'optional', PCLZIP_OPT_REMOVE_ALL_PATH => 'optional', PCLZIP_OPT_ADD_PATH => 'optional', PCLZIP_CB_PRE_ADD => 'optional', PCLZIP_CB_POST_ADD => 'optional', PCLZIP_OPT_NO_COMPRESSION => 'optional', PCLZIP_OPT_COMMENT => 'optional', PCLZIP_OPT_TEMP_FILE_THRESHOLD => 'optional', PCLZIP_OPT_TEMP_FILE_ON => 'optional', PCLZIP_OPT_TEMP_FILE_OFF => 'optional']);
                if ($v_result != 1) {
                    return 0;
                }
            } else {
        ...
        $v_supported_attributes = [PCLZIP_ATT_FILE_NAME => 'mandatory', PCLZIP_ATT_FILE_NEW_SHORT_NAME => 'optional', PCLZIP_ATT_FILE_NEW_FULL_NAME => 'optional', PCLZIP_ATT_FILE_MTIME => 'optional', PCLZIP_ATT_FILE_CONTENT => 'optional', PCLZIP_ATT_FILE_COMMENT => 'optional'];
    ...
    //        $v_path = "./";
        ...
        if ($v_size > 0) {
            ...
            if ((is_integer($v_arg_list[0])) && ($v_arg_list[0] > 77000)) {
                // ----- Parse the options
                $v_result = $this->privParseOptions($v_arg_list, $v_size, $v_options, [PCLZIP_OPT_PATH => 'optional', PCLZIP_OPT_REMOVE_PATH => 'optional', PCLZIP_OPT_REMOVE_ALL_PATH => 'optional', PCLZIP_OPT_ADD_PATH => 'optional', PCLZIP_CB_PRE_EXTRACT => 'optional', PCLZIP_CB_POST_EXTRACT => 'optional', PCLZIP_OPT_SET_CHMOD => 'optional', PCLZIP_OPT_BY_NAME => 'optional', PCLZIP_OPT_BY_EREG => 'optional', PCLZIP_OPT_BY_PREG => 'optional', PCLZIP_OPT_BY_INDEX => 'optional', PCLZIP_OPT_EXTRACT_AS_STRING => 'optional', PCLZIP_OPT_EXTRACT_IN_OUTPUT => 'optional', PCLZIP_OPT_REPLACE_NEWER => 'optional', PCLZIP_OPT_STOP_ON_ERROR => 'optional', PCLZIP_OPT_EXTRACT_DIR_RESTRICTION => 'optional', PCLZIP_OPT_TEMP_FILE_THRESHOLD => 'optional', PCLZIP_OPT_TEMP_FILE_ON => 'optional', PCLZIP_OPT_TEMP_FILE_OFF => 'optional']);
                if ($v_result != 1) {
                    return 0;
                }
    ...
    public function delete()
    {
        ...
        if ($v_size > 0) {
            ...
            // ----- Parse the options
            $v_result = $this->privParseOptions($v_arg_list, $v_size, $v_options, [PCLZIP_OPT_BY_NAME => 'optional', PCLZIP_OPT_BY_EREG => 'optional', PCLZIP_OPT_BY_PREG => 'optional', PCLZIP_OPT_BY_INDEX => 'optional']);
            if ($v_result != 1) {
                    return 0;
            }
        }
    ...
    public function privCalculateStoredFilename(&$p_filedescr, &$p_options)
    {
        ...
        if (isset($p_filedescr['new_full_name'])) {
            ...
            if ($p_remove_all_dir) {
                ...
                if (!str_ends_with($p_remove_dir, '/')) {
                    $p_remove_dir .= "/";
                }

                if ((str_starts_with($p_filename, "./")) || (str_starts_with($p_remove_dir, "./"))) {
                    if ((str_starts_with($p_filename, "./")) && (!str_starts_with($p_remove_dir, "./"))) {
    ...
    public function privExtractByRule(&$p_file_list, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
    {
        ...
        // ----- Check the path
        if (($p_path == "") || ((!str_starts_with($p_path, "/")) && (!str_starts_with($p_path, "../")) && (substr($p_path, 1, 2)!=":/"))) {
            $p_path = "./".$p_path;
        }
        ...
        for ($i=0, $v_nb_extracted=0; $i<$v_central_dir['entries']; $i++) {
            ...
            if ((isset($p_options[PCLZIP_OPT_BY_NAME])) && ($p_options[PCLZIP_OPT_BY_NAME] != 0)) {
                ...
                for ($j=0; ($j<sizeof($p_options[PCLZIP_OPT_BY_NAME])) && (!$v_extract); $j++) {
                    // ----- Look for a directory
                    if (str_ends_with($p_options[PCLZIP_OPT_BY_NAME][$j], "/")) {
                        // ----- Look if the directory is in the filename path
                        if ((strlen($v_header['stored_filename']) > strlen($p_options[PCLZIP_OPT_BY_NAME][$j])) && (str_starts_with($v_header['stored_filename'], $p_options[PCLZIP_OPT_BY_NAME][$j]))) {
                            $v_extract = true;
                        }
    ...
    //            eval('$v_result = '.$p_options[PCLZIP_CB_PRE_EXTRACT].'(PCLZIP_CB_PRE_EXTRACT, $v_local_header);');
        ...
        if ($p_entry['status'] == 'ok') {
            ...
            if (file_exists($p_entry['filename'])) {
                ...
                if ((($p_entry['external']&0x00000010)==0x00000010) || (str_ends_with($p_entry['filename'], '/'))) {
        ...
        for ($i=0, $v_nb_extracted=0; $i<$v_central_dir['entries']; $i++) {
            ...
            if ((isset($p_options[PCLZIP_OPT_BY_NAME])) && ($p_options[PCLZIP_OPT_BY_NAME] != 0)) {
                ...
                for ($j=0; ($j<sizeof($p_options[PCLZIP_OPT_BY_NAME])) && (!$v_found); $j++) {
                    ...
                    if (str_ends_with($p_options[PCLZIP_OPT_BY_NAME][$j], "/")) {
                        // ----- Look if the directory is in the filename path
                        if ((strlen($v_header_list[$v_nb_extracted]['stored_filename']) > strlen($p_options[PCLZIP_OPT_BY_NAME][$j])) && (str_starts_with($v_header_list[$v_nb_extracted]['stored_filename'], $p_options[PCLZIP_OPT_BY_NAME][$j]))) {
                            $v_found = true;
    ...
    public function privDirCheck($p_dir, $p_is_dir = false)
    {
        ...
        // ----- Remove the final '/'
        if (($p_is_dir) && (str_ends_with($p_dir, '/'))) {
...
function PclZipUtilPathInclusion($p_dir, $p_path)
{
    ...
    if (($p_dir == '.') || ((strlen($p_dir) >=2) && (str_starts_with($p_dir, './')))) {
        $p_dir = PclZipUtilTranslateWinPath(getcwd(), false).'/'.substr($p_dir, 1);
    }
    if (($p_path == '.') || ((strlen($p_path) >=2) && (str_starts_with($p_path, './')))) {
...
function PclZipUtilTranslateWinPath($p_path, $p_remove_disk_letter = true)
{
    if (stristr(php_uname(), 'windows')) {
        ...
        if ((strpos($p_path, '\\') > 0) || (str_starts_with($p_path, '\\'))) {
            $p_path = strtr($p_path, '\\', '/');
        }
    }
    return $p_path;
